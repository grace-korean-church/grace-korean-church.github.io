...
- 종료 코드: 성공 0. 피드를 못 받거나 파싱에 실패하면 0이 아닌 값으로 끝내고
  기존 파일은 건드리지 않는다.
- **피드 서버가 불안정하다.** 구현 중(2026-09-11) 같은 요청에 200·404·500이
  무작위로 섞여 나왔고, 한때는 모든 채널의 피드가 404였다. 집 회선과 GitHub
  러너 모두에서 재현됐다. 대응:
  - 채널 피드(`channel_id=UC…`)와 업로드 재생목록 피드(`playlist_id=UU…`)
    두 주소를 번갈아 시도한다. 형식은 같다.
  - 6회차, 회차 간 10·20·40·80·160초 대기(약 5분). 연결·응답 타임아웃 20초.
  - 그래도 실패하면 1로 종료해 Actions 탭에 실패로 남긴다. 화요일 재실행이
    한 번 더 잡고, 피드가 최근 15편을 주므로 몇 주 놓쳐도 다음 성공 때 다
    들어온다.
  - 몇 주 지켜봐도 계속 실패하면 YouTube Data API(API 키 필요)로 바꾼다.
...
- 게시 일시(UTC)를 `America/Chicago`로 바꾸면 예배 당일(일요일)이 된다.
  Liquid의 `date` 필터는 문자열에 붙은 `+00:00` 오프셋을 그대로 쓰므로 바로
  포맷하면 UTC 날짜(월요일)가 나온다. 대신 `date: "%s" | plus: 0 | date: …`로
  에포크 정수를 거치면 `Time.at`이 `_config.yml`의 `timezone`을 따르고 DST도
  맞는다. 확인: `2026-09-07T04:50:36+00:00` → 9월 6일, `2026-01-12T05:10:00+00:00` → 1월 11일.
